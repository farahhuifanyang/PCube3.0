    ...
    def singlePass(self, corpus, texts, theta):
        ...
        for vector, text in zip(corpus[-len(texts):], texts):
            if num_topic == 0:
                ...
                file_name[num_topic] = []
                file_name[num_topic].append([text['id'],text['time']])
                ...
                if max_value >= theta:
                    ...
                    file_name[max_index].append([text['id'],text['time']])
...
if __name__ == '__main__':
    ...
    # single_pass_cluster.run(theta = args.theta, n=args.number, datMat = datMat)
