...
Author: Huifan Yang
Date: 2021-06-02
LastEditTime: 2021-06-05 11:47:26
LastEditors: Please set LastEditors
Description: Define algorithm interface
FilePath: /PCube3.0/coreNLP/ORE/dataParser.py
'''
...
def read_squad_examples(input_json, is_training, version_2_with_negative):
...
def evaluate(args, input_json, model, tokenizer, prefix=""):
    dataset, examples, features = load_and_cache_examples(args, input_json, tokenizer, evaluate=True, output_examples=True)
...
def load_and_cache_examples(args, input_json, tokenizer, evaluate=False, output_examples=False):
    if args.local_rank not in [-1, 0] and not evaluate:
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache

    # Load data features from cache or dataset file
    cached_features_file = args.ORE_output_path + 'cached_{}_{}_{}'.format(
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
        ...
        examples = read_squad_examples(input_json=input_json,
